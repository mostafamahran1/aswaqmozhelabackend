...
from .models import AccessoriesProduct, AccessoriesReview
...
def get_by_id_product(request, pk):
    product = get_object_or_404(AccessoriesProduct.objects.prefetch_related('variants'), id=pk)
...
def new_product(request):
    ...
    if serializer.is_valid():
        product = AccessoriesProduct.objects.create(
            name=data['name'],
        ...
        if variants_data:
            try:
                ...
                content_type = ContentType.objects.get_for_model(AccessoriesProduct)
...
def create_review(request, pk):
    ...
    if data['rating'] <= 0 or data['rating'] > 10:
        ...
        AccessoriesReview.objects.create(
            user=user,
            product=product,
            rating=data['rating'],
