...
from .models import SheinProduct, SheinReview
...
def get_all_products(request):
    ...
    queryset = SheinProduct.objects.filter(is_active=True)\
    ...
    # حساب العدد الإجمالي للمنتجات المفلترة (سريع وخفيف على قاعدة البيانات)
...
def get_by_id_product(request, pk):
    product = get_object_or_404(SheinProduct.objects.prefetch_related('variants'), id=pk)
...
def new_product(request):
    ...
    if serializer.is_valid():
        product = SheinProduct.objects.create(
            name=data['name'],
        ...
        if variants_data:
            try:
                ...
                content_type = ContentType.objects.get_for_model(SheinProduct)
...
def update_product(request, pk):
    product = get_object_or_404(SheinProduct, id=pk)
