...
from .models import SpicesProduct, SpicesReview
...
def get_all_products(request):
    ...
    queryset = SpicesProduct.objects.filter(is_active=True)\
...
def get_by_id_product(request, pk):
    product = get_object_or_404(SpicesProduct.objects.prefetch_related('variants'), id=pk)
    ...
    if serializer.is_valid():
        ...
        if variants_data:
            try:
                ...
                content_type = ContentType.objects.get_for_model(SpicesProduct)
...
def update_product(request, pk):
    product = get_object_or_404(SpicesProduct, id=pk)
