...
from .models import GiftsProduct, GiftsReview
...
def get_by_id_product(request, pk):
    product = get_object_or_404(GiftsProduct.objects.prefetch_related('variants'), id=pk)
...
def new_product(request):
    ...
    if serializer.is_valid():
        product = GiftsProduct.objects.create(
            name=data['name'],
        ...
        if variants_data:
            try:
                ...
                content_type = ContentType.objects.get_for_model(GiftsProduct)
...
def update_product(request, pk):
    product = get_object_or_404(GiftsProduct, id=pk)
