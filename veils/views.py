...
from .models import VeilsProduct, VeilsReview
...
def get_all_products(request):
    ...
    queryset = VeilsProduct.objects.filter(is_active=True)\
...
def get_by_id_product(request, pk):
    product = get_object_or_404(VeilsProduct.objects.prefetch_related('variants'), id=pk)
...
def new_product(request):
    ...
    if serializer.is_valid():
        product = VeilsProduct.objects.create(
            name=data['name'],
        ...
        if variants_data:
            try:
                ...
                content_type = ContentType.objects.get_for_model(VeilsProduct)
...
def update_product(request, pk):
    product = get_object_or_404(VeilsProduct, id=pk)
