...
from .models import ToysProduct, ToysReview
...
def get_all_products(request):
    ...
    queryset = ToysProduct.objects.filter(is_active=True)\
...
def get_by_id_product(request, pk):
    product = get_object_or_404(ToysProduct.objects.prefetch_related('variants'), id=pk)
...
def new_product(request):
    ...
    if serializer.is_valid():
        product = ToysProduct.objects.create(
            name=data['name'],
        ...
        if variants_data:
            try:
                ...
                content_type = ContentType.objects.get_for_model(ToysProduct)
...
def update_product(request, pk):
    product = get_object_or_404(ToysProduct, id=pk)
