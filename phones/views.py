...
from .models import PhonesProduct, PhonesReview
...
def get_all_products(request):
    ...
    # 1. جلب المنتجات النشطة أولاً بدون ترتيب عشوائي لضمان سرعة الفلترة والعد
    queryset = PhonesProduct.objects.filter(is_active=True)\
    ...
    # حساب العدد الإجمالي للمنتجات المفلترة (سريع جداً وخفيف على قاعدة البيانات)
...
def get_by_id_product(request, pk):
    product = get_object_or_404(PhonesProduct.objects.prefetch_related('variants'), id=pk)
...
def new_product(request):
    ...
    if serializer.is_valid():
        product = PhonesProduct.objects.create(
            name=data['name'],
        ...
        if variants_data:
            try:
                ...
                content_type = ContentType.objects.get_for_model(PhonesProduct)
...
def update_product(request, pk):
    product = get_object_or_404(PhonesProduct, id=pk)
