...
from .models import SupermarketProduct, SupermarketReview
...
def get_all_products(request):
    ...
    queryset = SupermarketProduct.objects.filter(is_active=True)\
...
def get_by_id_product(request, pk):
    product = get_object_or_404(SupermarketProduct.objects.prefetch_related('variants'), id=pk)
...
def new_product(request):
    ...
    if serializer.is_valid():
        product = SupermarketProduct.objects.create(
            name=data['name'],
        ...
        if variants_data:
            try:
                ...
                content_type = ContentType.objects.get_for_model(SupermarketProduct)
...
def update_product(request, pk):
    product = get_object_or_404(SupermarketProduct, id=pk)
...
def create_review(request, pk):
    ...
    if data['rating'] <= 0 or data['rating'] > 10:
        ...
        SupermarketReview.objects.create(
            user=user,
            product=product,
            rating=data['rating'],
