...
def get_all_products(request):
    ...
    # حساب العدد الإجمالي للمنتجات المفلترة (هنا سريع جداً وبدون جهد على الداتابيز)
...
def get_by_id_product(request, pk):
    product = get_object_or_404(FoodsProduct.objects.prefetch_related('variants'), id=pk)
    ...
    if serializer.is_valid():
        ...
        if variants_data:
            try:
                ...
                content_type = ContentType.objects.get_for_model(FoodsProduct)
